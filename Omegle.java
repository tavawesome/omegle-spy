...
{
	public static final Pattern str_regex =
		Pattern.compile("(\")((?>(?:(?>[^\"\\\\]+)|\\\\.)*))\\1");
	public static final Pattern escape_regex =
		Pattern.compile("\\\\([\'\"\\\\bfnrt]|u(....))");
	...
	public boolean start ()
	{
		...
		if (m.matches())
		{
			chatId = m.group(2); // 2 is the actual string
		}
		else
		{
			return false;
	...
	public void run ()
	{
		String eventr;
		while (chatId != null &&
			   (eventr = wget(events_url, true, "id", chatId)) != null &&
			   /* chatId != null && /* hahahaha i have to check it twice */
			   !eventr.equals("null"))
		{
			/* SUPER DUPER SUPER SUPER LOLZ-TACULAR HACKY
			   THIS IS PROBABLY THE MOST HACKY THING ANYONE HAS EVER
			   DONE IN THE HISTORY OF HACK-VILLES */
			...
			for (List<String> ev : events)
			{
				...
				for (OmegleListener ol : listeners)
					ol.eventFired(this, name, args);
			}
		}
		if (chatId != null)
		{
			//means connection asploded?
			chatId = null;
			//may want to act appropriately
		}
	}
	...
	public boolean disconnect ()
	{
		...
		{
			/* ohnoes, i pooped all over the place. PUT IT BACK PUT IT BACK */
